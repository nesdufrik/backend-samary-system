...
export const listAllItemsController = async (
	...
	try {
		...
		const cat = req.query.cat ? (req.query.cat as string) : 'all'
		const sub = req.query.sub ? (req.query.sub as string) : 'all'
		const response = await getAllItems(req.params.sucursalId, page, cat, sub)
		res.send({
			success: true,
			data: response,
		})
...
export const createItemController = async (
	...
	try {
		const response = await createItem(req.body, req.file, req.params.sucursalId)
		res.send({
			success: true,
			data: response,
		})
