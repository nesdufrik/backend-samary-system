...
export const loginAuth = async (data: LoginAuthType) => {
    let permissions = ''
    let logInAdmin
    let checkIs: any = await UsuarioModel.findOne({ email: data.email })
    ...
    if (!checkIs) {
        ...
        if (!checkIs) {
            throw new ApiError(
                401,
                `Por favor revisa tu usuario y contraseña. Los campos son sensibles a mayúsculas y minúsculas`
            )
        }
...
export const oauthLogin = async (id: string) => {
    const checkIs = await EmpleadoModel.findOne(
        { _id: id },
        { fullName: 1, avatar: 1 }
    )
        .populate('sucursal', '-_id -createdAt -updatedAt -empresa')
